<script>
    ...
    import { currentStep, customerEmail, customerName, customerPassword, emailIsError, nameIsError, passwordIsError } from "../lib/sharedState";
    import {customerLastnames, customerMatricula, lastnamesIsError, matriculaIsError } from "../lib/sharedState";
    import { customerGroup, customerCarreer, customerAlergy, groupIsError, carreerIsError, alergyIsError } from "../lib/sharedState";
    ...
    <main>
        ...
        <footer>
            <button class="previousButton" on:click={()=>{$currentStep--}} class:show="{$currentStep > 1 && $currentStep < 5}"> Regresar </button>
            {#if $currentStep < 4}
